...
const hasPlates = computed(() => {
  return props.leftTableData.some(item => item.plates && item.plates.length > 0)
})

function handleRowClick(row: DynamicBacktrackItem) {
  emit('update:selectedStockCode', row.stock_code)
}

function getRowClassName({ row }: { row: DynamicBacktrackItem }) {
